...
export interface AudioStorage {
  saveAudio(buf: Uint8Array, ext: string): Promise<{ base: string; pathOnDisk: string }>
  runDailyCleanupIfNeeded(): Promise<void>
}
...
export function createAudioStorage(dataDir: string): AudioStorage {
  const RECORDINGS_DIR = join(dataDir, 'recordings')
  const CLEANUP_STATE = join(dataDir, '.last-cleanup')
