...
> - **Telegram bot (optional, owner-only):** [`src/telegram-bot.ts`](src/telegram-bot.ts) — a **standalone** long-poll bot (raw Telegram HTTP over `fetch`, no `grammy` dep). Forward it a voice/audio message → it replies with the transcript as a tap-to-copy `<code>` block. Deliberately isolated: **no DB, no history, no cost tracking, no Mac clipboard fan-out** — it only reuses `transcribeAudio`. Owner lock via `TELEGRAM_ALLOWED_USER_IDS` (empty allowlist → "setup mode": replies to any sender with their own Telegram ID so you can populate the env var). Starts from `src/index.ts` only when `TELEGRAM_BOT_TOKEN` is set; unset → never runs. Config in `src/config.ts`, env in `.env.example`, tests in `tests/telegram-bot.test.ts`.
>
